...
public extension UIView {
    private static let relatedControl = WeakDictionary<UIView, UIControl>()
    ...
    var relatedControl: UIControl? {
        get {
            return UIView.relatedControl[self] ?? self as? UIControl
        }
        set(value) {
            UIView.relatedControl[self] = value
        }
    }
        ...
        set(value) {
            ...
            if previousDoubleNillable == nil {
                self.onLayoutSubviews.append { view in
                    CATransaction.withDisabledActions {
                        view.backgroundLayer?.frame = view.bounds
                    }
                    return false
                }
            }
